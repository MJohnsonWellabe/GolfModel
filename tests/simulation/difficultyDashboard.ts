...
export function buildDashboard(p: DashboardPayload): string {
  ...
  <div class="note"><b>Putting vs tour — a real tension.</b> Tour: ~1.7–1.8 putts/hole, ~40% one-putt, ~3% three-putt for a good player. Putting here is a touch <i>more</i> forgiving (Good user ~1.5, Expert ~1.5) because on these hard courses <b>birdies come almost entirely from made putts</b> (perfect approaches still carry ~3% physics depth-noise → ~15ft, so approach proximity can't be tightened without touching physics/courses). Reaching the owner's <b>Expert+Good good-round = −3</b> therefore <i>requires</i> forgiving putts — with tour-realistic putting the expert good-round only reaches −2. The two targets conflict; the score target (primary, explicit) was prioritised. One-putt/three-putt ordering is monotonic by skill, and putting is clearly the most forgiving element (owner intent).</div>
